...
export default function DetailBar() {
  ...
  const { data: isFav, isLoading:isFavLoading, error:isFavError,refetch } = isfavorited(id, idUser);
  useEffect(() => {
    if (isFavLoading) return; // Ne rien faire si les données sont en cours de chargement
    if (isFavError) {
      console.error('Erreur lors de la vérification du statut de favori :', isFavError);
      return; // Vous pourriez aussi gérer l'erreur ici
    }
    console.log("favori ?" + isFav);
    // Mettre à jour la couleur en fonction du statut de favori
    setFavColor(isFav ? 'yellow' : 'black');
  }, [isFav,isFavLoading,isFavError,id,idUser]); // Depend on id and idUser to re-check when they change
  useEffect(() => {
    // Refetch lorsque id ou idUser change
    refetch();
  }, [id, idUser, refetch]);
  useEffect(() => {
    // Reset color if id changes
    if (id) {
        setFavColor('black'); // Optionally reset to default color
    }
    if(isFav){
        setFavColor('yellow');
    }
  ...
  const refetchFavorites = async () => {
    // Call your fetching logic again to get updated favorite status
    ...
    setFavColor(newFav ? 'yellow' : 'black'); // Update the favorite color based on new data
};
    ...
    <ScrollView flex={1} alwaysBounceVertical={true}>
      ...
      <YStack marginTop={15} gap={25}>
        ...
        <XStack gap={'$3'} marginLeft={15}>
          <FontAwesomeIcon icon={faLocationDot} color="#05123B" size={18} />
          <SizableText style={{ fontFamily: 'Gotham-Medium' }} color={"black"} size={'$5'} flexShrink={1}>
            {bar.address}
          </SizableText>
        </XStack>
        <XStack gap={'$3'} marginLeft={15}>
          <FontAwesomeIcon icon={faFilter} color="#05123B" size={18} />
        ...
        <XStack gap={'$3'} marginLeft={15}>
          <FontAwesomeIcon icon={faPenToSquare} color="#05123B" size={18} />
          <Paragraph size="$5" fontWeight="800" style={{ fontFamily: 'Gotham-Medium' }} flexShrink={1} color={"black"}>
            {bar.description}
          </Paragraph>
        </XStack>
        <XStack gap={'$3'} marginLeft={15}>
          <FontAwesomeIcon icon={faClock} color="#05123B" size={18} />
          <SizableText style={{ fontFamily: 'Gotham-Medium' }} color={"black"} size={'$5'} flexShrink={1}>
      ...
      <YStack marginTop={15} gap={25}>
        ...
        <Pressable onPress={clickFav}>
          <FontAwesomeIcon icon={faStar} style={{alignSelf:"center"}} size={30} color={favColor} />
        </Pressable>
      </YStack>
